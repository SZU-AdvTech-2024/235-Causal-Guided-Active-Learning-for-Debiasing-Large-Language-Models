...
def get_args():
    ...
    parser.add_argument('--log_name', type=str, default="zs_0.2")
        ...
        if args.debias:
            for prompt in prompt_raw[args.dataset][1:2]:
                for prompt_debias in prompt_debias_set[args.model][args.dataset][1:2]:
                    acc = inference_model.predict(prompt,debias_prompt=prompt_debias,save_name=args.log_name)
            ...
            for prompt in prompt_raw[args.dataset][1:2]:
                acc = inference_model.predict(prompt,save_name=args.log_name)
                args.logger.info(f"Prompt: {prompt}, acc: {acc}%\n")
                with open(RESULTS_DIR+args.save_file_name+".txt", "a+") as f:
                    f.write("Prompt: {}, acc: {:.2f}%\n".format                                                                                                                                           (prompt, acc*100))
    else:
        if args.fs_num!=-1:
            for prompt in prompt_raw[args.dataset][:1]:
                for i in range(args.fs_num):
                    acc = inference_model.predict(prompt,fs_num=i,save_name=args.log_name)
                    args.logger.info(f"Prompt: {prompt}, acc: {acc}%\n")
                    with open(RESULTS_DIR+args.save_file_name+".txt", "a+") as f:
                        f.write("Prompt: {}, acc: {:.2f}%\n".format(prompt, acc*100))
        else:
