...
class Inference(object):
    ...
    def create_model(self):
        ...
        if self.model not in ['chatgpt', 'gpt4']:
            ...
            if self.model.lower() == 'llama2-13b-chat':
                ...
                model_dir = os.path.join(self.args.model_dir, self.model)
                print(model_dir)
                self.tokenizer = AutoTokenizer.from_pretrained(model_dir, device_map="auto", padding_side="left")
                self.pipe = AutoModelForCausalLM.from_pretrained(model_dir, device_map="auto", torch_dtype=torch.float16)
    ...
    def predict(self, prompt=None, debias_prompt='', fs_num=None, save_name="1121"):
        if self.model in ["chatgpt", "gpt4"]:
            results = self.predict_by_openai_api(self.model, prompt, debias_prompt, save_name)
        else:
            results = self.predict_by_local_inference(self.model, prompt, debias_prompt,fs_num=fs_num, save_name=save_name)
        return results

    def predict_by_openai_api(self, model, prompt, debias_prompt, save_name):
    ...
    def predict_by_local_inference(self, model, prompt, debias_prompt, fs_num=None, save_name=""):
        data_len = len(self.args.data)
        ...
        if self.args.dataset in ['mnli','chatbot'] and self.args.shot==0 and not self.args.debias:
            ...
            if 'vicuna' in self.model:
                write_jsonl('./data/'+self.args.dataset+'/vicuna/examples_{}.jsonl'.format(save_name), examples)
            else:
                write_jsonl('./data/' + self.args.dataset + '/llama2/examples_{}.jsonl'.format(save_name), examples)

        if self.args.dataset in ['chatbot', 'mt_bench']:
            if 'vicuna' in self.model:
                #write_jsonl('./data/' + self.args.dataset + '/vicuna/pred_reverse.jsonl', (preds, gts))
                write_jsonl('./data/' + self.args.dataset + '/vicuna/pred_origin_{}.jsonl'.format(save_name), (preds, gts))
            else:
                #write_jsonl('data/' + self.args.dataset + '/llama2/pred_reverse.jsonl', (preds, gts))
                write_jsonl('./data/' + self.args.dataset + '/llama2/pred_origin_{}.jsonl'.format(save_name), (preds, gts))
    ...
    def pred_by_generation(self, inputs, model):
        ...
        # assert self.tokenizer.padding_side == 'left', "Padding side must be 'left' before generation."
        print(self.tokenizer.padding_side)
        outputs = self.pipe.generate(**inputs,temperature=1e-5,early_stopping=True,return_dict_in_generate=True,output_scores=True,output_hidden_states=True)
        return outputs
